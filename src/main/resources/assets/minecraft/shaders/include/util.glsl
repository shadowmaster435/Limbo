...
bool range_equals(float a, float b, float range) {
...
//
//float line_distance(vec3 p1, vec3 p2, vec3 p3) {
//    float b = Math.sqrt(Math.pow((p2.x - p3.x), 2)+ Math.pow((p2.y - p3.y), 2)+ Math.pow((p2.z - p3.z), 2));
//    float S = Math.sqrt(Math.pow((p2.y - p1.y) * (p3.z - p1.z) - (p2.z - p1.z) * (p3.y - p1.y), 2) + Math.pow((p2.z - p1.z) * (p3.x - p1.x) - (p2.x - p1.x) * (p3.z - p1.z), 2) + Math.pow((p2.x - p1.x) * (p3.y - p1.y) - (p2.y - p1.y) * (p3.x - p1.x), 2)) / 2;
//    return 2 * S / b;
//}
