...
void main()
{
    ...
    float d = distance(pixelated, Center);

    vec4 texfloor = texture2D(Floor, vec2(pixelated.x + (pixelated.y - pixelated.z),  pixelated.z + (pixelated.y) ));

    vec3 col = fragColor.rgb = vec3(snoise(vec4(pixelated, STime)) + 0.5, 0, 0)  * sin(sceneDepth * 3.14);
    if (pixelPosition.y < -47.0625) {
    ...
    if (sin(sceneDepth * 3.14) < 0.01 && Center.y < -47) {
        fragColor.rgb = col;

    }
    if (pixelPosition.y > -47.0625 && pixelPosition.y < -46.9 && sin(sceneDepth * 3.14) > 0.0025) {
        fragColor.rgb = vec3(1.0);
    }
}