...
public abstract class ModGlImportProcessor {
    /**
     ...
     */
        ...
        private static final Pattern INCLUDE_IMPORT_PATTERN = Pattern.compile(
                "(#(?:/\\*(?:[^*]|\\*+[^*/])*\\*+/|\\h)*include(?:/\\*(?:[^*]|\\*+[^*/])*\\*+/|\\h)*(?:\"(.*)\"|<(.*)>))"
        );
        ...
        private List<String> parseImports(String source, Context context, String path) {
            ...
            Matcher matcher = INCLUDE_IMPORT_PATTERN.matcher(source);
                ...
                if (!hasBogusString(source, matcher, j)) {
                    ...
                    if (string2 != null) {
                        ...
                        if (!Strings.isNullOrEmpty(string5)) {
                            ...
                            String string6 = bl ? String.format(Locale.ROOT, "/*#inclide \"%s\"*/", string2) : String.format(Locale.ROOT, "/*#inclide <%s>*/", string2);
